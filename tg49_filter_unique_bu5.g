...
#
# Supported input list variables (as seen in your files):
#   - recs             := [ rec(size:=..., gen:=[...]), ... ];
#   - TG49_Candidates  := [ rec(sizeG:=..., gens:=[...]), ... ];
#   - TG49_REPS        := [ rec(gens:="perm ; perm ; ...", ...), ... ];
#   - TG49_B_RECS     := [ rec(sizeG:=..., gens:=[...]), ... ];
#
# Output file defines:
...
#############################################################################
# Global variable declarations (avoid "Unbound global variable" warnings)
#############################################################################
#############################################################################
# String helpers
#############################################################################
...
#############################################################################
# Load list variable from a file (robust, avoids Unbind/read-only issues)
#############################################################################
TG49_FirstListVarNameInFile := function(f)
  local inp, line, s, pos, pos2, name;
  ...
  while true do
    ...
    if s = "" then
      ...
      # Pattern 1: BindGlobal("NAME", ...)
      pos := PositionSublist(s, "BindGlobal(\"");
      if pos <> fail then
        pos := pos + Length("BindGlobal(\"");
        pos2 := Position(s, '"', pos);
        if pos2 <> fail and pos2 > pos then
          name := s{[pos..pos2-1]};
          CloseStream(inp);
          return name;
        fi;
...
TG49_LoadListFromFile := function(f)
  ...
  var := TG49_FirstListVarNameInFile(f);
  Read(f);

  if var <> fail and IsBoundGlobal(var) then
    L := ValueGlobal(var);
    if IsList(L) then
      return L;
    fi;
  ...
  # Fallback: try known variable names (using globals to avoid warnings)
  if IsBoundGlobal("recs") and IsList(ValueGlobal("recs")) then return ValueGlobal("recs"); fi;
  if IsBoundGlobal("TG49_Candidates") and IsList(ValueGlobal("TG49_Candidates")) then return ValueGlobal("TG49_Candidates"); fi;
  if IsBoundGlobal("TG49_B_RECS") and IsList(ValueGlobal("TG49_B_RECS")) then return ValueGlobal("TG49_B_RECS"); fi;
  if IsBoundGlobal("TG49_REPS") and IsList(ValueGlobal("TG49_REPS")) then return ValueGlobal("TG49_REPS"); fi;
  if IsBoundGlobal("tg49_reps") and IsList(ValueGlobal("tg49_reps")) then return ValueGlobal("tg49_reps"); fi;
